      ...
      <h1 className="PIT"   >Product In Trend</h1>
        {productItems.map((productItem)=>(
          ...
          <img src={productItem.img} alt="" />
             <hr style={{color:"#0d0221" ,width:"100%", opacity:"1"}}/>
             <h6 className='brandname'>{productItem.brand}</h6>
            <h3 className="prodtitle" >{productItem.name}</h3>
            <p className='price'>₹{productItem.price}/-</p>
            <button  className="btnprd" onClick={()=>handleAddProduct(productItem)}>Add to Bag <ShoppingBag/></button>
          </div>
          </div>
      </div>
        ))}
